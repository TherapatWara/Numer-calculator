...
class Gauss_Jordan extends React.Component
{
    constructor(props)
    {
        ...
        this.Gauss_JordanCal = this.Gauss_JordanCal.bind(this)
    }
    ...
    Gauss_JordanCal(size_array,val,show_web)
    {
      var a = [[]]
      var k=0,i=0,j=0
    ...
    render(){
        return(
          ...
          {this.state.val.map((row, rowIndex) => (
                ...
                <br></br>
               <Button className= "botton" onClick={()=>this.Gauss_JordanCal(this.state.size_array, this.state.val, this.state.show_web)}>Calulate</Button>
               </Container>
